...
class JumpCNF(nn.Module):
    ...
    def __init__(
        ...
        self.inst_flow = flow_layers.HypernetworkRadialFlow(
            dim, cond_dim=1 + dim + aux_dim, nflows=4
        )  # instant update
    ...
    def _cond_logliks(
        ...
        event_times = torch.cat(
            [torch.zeros(N, 1).to(event_times), event_times], dim=1
        )  # (N, 1 + T)
        ...
        for i in range(T):
            ...
            xs, dlogps = self.cnf.integrate(
                ...
                xs,
                dlogps,
                method="dopri5" if i < T - 1 and self.training else "dopri5",
                norm=norm_fn,
            )
            ...
            if i < T - 1:
                obs_x = (
                    spatial_locations[:, -i - 2]
                    .reshape(N, 1, D)
                    .expand(N, i + 1, D)
                    .reshape(-1, D)
                )
                obs_t = (
                    event_times[:, -i - 2].reshape(N, 1).expand(N, i + 1).reshape(-1, 1)
                )
                cond = torch.cat(
                    [obs_t, obs_x, auxs[:, -self.aux_dim :]], dim=1
                )  # (N * (i + 1), 1 + D + D_a)
            ...
            dlogps = torch.where(
                input_mask[:, -i - 1 :], dlogps, torch.zeros_like(dlogps)
            )

        logpz = gaussian_loglik(
            xs, self.z_mean.expand_as(xs), self.z_logstd.expand_as(xs)
        ).sum(
            -1
        )  # (N, T)
        logpx = logpz - dlogps
...
class ConditionalSequential(nn.Module):
    def __init__(self, input_layers, cond_layers, actfn_layers):
        super().__init__()
        assert len(input_layers) == len(cond_layers) and len(input_layers) - 1 == len(
            actfn_layers
        )
