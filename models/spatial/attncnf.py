...
class SelfAttentiveODEFunc(nn.Module):
    def __init__(
        ...
        self.self_attns = nn.ModuleList(
            [mha(self.embed_dim, num_heads=4) for _ in range(nblocks)]
        )
        self.attn_actnorms = nn.ModuleList(
            [ActNorm(self.embed_dim) for _ in range(nblocks)]
        )
    ...
    def forward(self, t, state, rm_nonself_grads=False):
        T, N, _ = self.shape
        x, a = (
            state[:, : self.dim],  # TxN, (D / self.aux_dim)
            state[:, max(self.dim + 1, state.shape[-1] - self.aux_dim) :],
        )
        x = torch.cat([x, a], dim=-1)  # TxN, (D+aux_dim)
        x = self.embedding(t, x)  # T x N, embed_dim
        x = x.reshape(T, N, self.embed_dim)

        attn_mask = self._create_self_attn_mask(T).to(x)  # upper triangle: -1e12
        for norm0, self_attn, gate0, norm1, fc, gate1 in zip(
            ...
            h, attn_weights = self_attn(
                norm0(x), attn_mask=attn_mask, rm_nonself_grads=rm_nonself_grads
            )
            # self._update_attn_weights(attn_weights)
            x = x + gate0(h)  # tanh
            x = x + gate1(fc(norm1(x)))  # tanh
...
class SelfAttentiveCNF(nn.Module):
    ...
    def _cond_logliks(
        self, event_times, spatial_locations, input_mask=None, aux_state=None
    ):
        """
        Args:
            event_times: (N, T)
        ...
        z, delta_logp = self.cnf.integrate(
            t0, t1, x, torch.zeros_like(event_times), norm=norm_fn
        )  # t_i +2 -> t_0 + 2
        z = z[:, : self.dim]  # (T * N, D)
        ...
        # t + 2 -> 0
        if aux_state is not None:
            cond_inputs = [aux_state[:, :, -self.aux_dim :], t_embed]  # 16+64
        else:
            cond_inputs = [t_embed]
        cond = torch.cat(cond_inputs, dim=-1)  # (N, T, -1)
        cond = torch.where(
            input_mask[..., None].expand_as(cond).bool(), cond, torch.zeros_like(cond)
        )
        cond = cond.transpose(0, 1).reshape(T * N, -1)

        z_params = self.base_dist_params(cond)  # Q
    ...
    def spatial_conditional_logprob_fn(
        self, t, event_times, spatial_locations, aux_state=None
    ):
        """
        Args:
            t: scalar
        ...
        def loglikelihood_fn(s):
            ...
            bsz_event_times = torch.cat(
                [bsz_event_times, torch.ones(bsz, 1).to(bsz_event_times) * t], dim=1
            )
            bsz_spatial_locations = spatial_locations[None].expand(bsz, T, D)
            bsz_spatial_locations = torch.cat(
                [bsz_spatial_locations, s.reshape(bsz, 1, D)], dim=1
            )
