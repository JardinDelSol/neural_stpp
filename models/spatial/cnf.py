...
class TimeVariableCNF(nn.Module):
    ...
    def integrate(
        ...
        if intermediate_states > 1:
            tt = torch.linspace(self.start_time, self.end_time, intermediate_states).to(
                t0
            )
        else:
...
def build_fc_odefunc(
    ...
    assert (
        layer_type in LAYERTYPES.keys()
    ), f"layer_type must be one of {LAYERTYPES.keys()} but was given {layer_type}"
