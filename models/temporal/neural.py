...
class SplitHiddenStateODEFunc(nn.Module):
    ...
    def update_state(self, t, tpp_state, cond=None):
        ...
        upd_c, upd_h = torch.split(
            self.update_net(t, inputs), tpp_state.shape[1] // 2, dim=1
        )
...
class NeuralPointProcess(TemporalPointProcess):
    ...
    def __init__(
        ...
        for i in range(separate):
            ...
            if style in ["split", "simple"]:
                update_net = construct_diffeqnet(
                    hidden_dims[0] // separate + cond_dim,
                    hidden_dims[1:],
                    hidden_dims[0] // separate,
                    time_dependent=False,
                    actfn="celu",
        ...
        intensity_odefunc = IntensityODEFunc(
            self.hdim, self.hidden_state_dynamics, intensity_net
        )
    ...
    def logprob(self, event_times, spatial_locations, input_mask, t0, t1):
        intensities, Lambda, _ = self.integrate_lambda(
            event_times, spatial_locations, input_mask, t0, t1
        )
        log_intensities = torch.log(intensities + 1e-8)
        log_intensities = torch.where(
            input_mask.bool(), log_intensities, torch.zeros_like(log_intensities)
        )
    ...
    def integrate_lambda(
        self, event_times, spatial_location, input_mask, t0, t1, nlinspace=1
    ):
        """
        Args:
            event_times: (N, T)
        ...
        for i in range(T):
            ...
            if i < T - 1 or t1 is not None:
                ...
                updated_tpp_state = self.hidden_state_dynamics.update_state(
                    event_times[:, i], tpp_state, cond=cond
                )
                tpp_state = torch.where(
        ...
        prejump_hidden_states = torch.cat(prejump_hidden_states, dim=0).transpose(
            0, 1
        )  # (N, T * nlinspace, D)
...
def construct_diffeqnet(
    ...
    linear_fn = (
        diffeq_layers.IgnoreLinear if time_dependent else diffeq_layers.ConcatLinear_v2
    )
