...
def _main(rank, world_size, args, savepath, logger):
    ...
    if args.model == "jumpcnf" and args.tpp == "neural":
        ...
        if args.model == "gmm":
            model = CombinedSpatiotemporalModel(
                GaussianMixtureSpatialModel(), tpp_model
            ).to(device)
        elif args.model == "cnf":
            model = CombinedSpatiotemporalModel(
                IndependentCNF(
    ...
    for name, p in model.named_parameters():
        if "self_attns" in name:
            attn_params.append(p)  # no attn for CNF
        else:
            params.append(p)

    optimizer = torch.optim.AdamW(
        [{"params": params}, {"params": attn_params}],
        lr=args.lr,
    ...
    for epoch in range(
        begin_epoch, math.ceil(args.num_iterations / len(train_epoch_iter))
    ):
        ...
        for batch in batch_iter:
            ...
            # Set learning rate
            total_itrs = math.ceil(args.num_iterations / len(train_epoch_iter)) * len(
                train_epoch_iter
            )
            ...
            grad_norm = torch.nn.utils.clip_grad.clip_grad_norm_(
                model.parameters(), max_norm=args.gradclip
            ).item()
            ...
            if itr % args.logfreq == 0:
                ...
                if rank == 0:
                    ...
                    tb_writer.add_scalar(
                        "train/time_per_itr", elapsed_time / args.logfreq, itr
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, choices=MAPS.keys(), default="earthquakes_jp"
    )
    ...
    parser.add_argument(
        "--tpp_actfn", type=str, choices=TPP_ACTFNS.keys(), default="softplus"
    )
    parser.add_argument("--hdims", type=str, default="64-64-64")
    parser.add_argument(
        "--layer_type", type=str, choices=["concat", "concatsquash"], default="concat"
    )
    parser.add_argument("--tpp_hdims", type=str, default="32-32")
    parser.add_argument("--tpp_nocond", action="store_false", dest="tpp_cond")
    parser.add_argument(
        "--tpp_style", type=str, choices=["split", "simple", "gru"], default="gru"
    )
    ...
    if (
        args.share_hidden
        and args.model in ["jumpcnf", "attncnf"]
        and args.tpp == "neural"
    ):
