...
class PlanarFlow(nn.Module):
    ...
    def forward(self, x, logpx=None, reverse=False, **kwargs):
        ...
        with torch.enable_grad():
            ...
            h = self.activation(torch.mm(x, self.w.view(self.nd, 1)) + self.b)
    ...
    def _logdetgrad(self, h, x):
        ...
        psi = grad(
            h,
            x,
            grad_outputs=self.one.expand_as(h).type_as(h).detach(),
            create_graph=True,
            only_inputs=True,
        )[0]
...
class RadialFlow(nn.Module):
    ...
    def forward(
        self, x, logpx=None, reverse=False, z0=None, log_alpha=None, beta=None, **kwargs
    ):
        if reverse:
            raise ValueError(f"{self.__class__.__name__} does not support reverse.")
        ...
        if logpx is not None:
            logdetgrad = (self.nd - 1) * torch.log(1 + beta * h) + torch.log(
                1 + beta * h - beta * r / (torch.exp(log_alpha) + r) ** 2
            )
...
class HypernetworkRadialFlow(nn.Module):
    def __init__(self, nd, cond_dim, nflows=1):
        ...
        self.radial_flows = nn.ModuleList(
            [RadialFlow(nd, hypernet=True) for _ in range(nflows)]
        )
    ...
    def forward(self, x, logpx=None, reverse=False, cond=None, **kwargs):
        ...
        for i in range(self.nflows):
            ...
            log_alpha = (
                hyper_out[:, start_ind + self.nd : start_ind + self.nd + 1] - 6.0
            )
            beta = hyper_out[:, start_ind + self.nd + 1 : start_ind + self.nd + 2] - 6.0
            out = self.radial_flows[i](
                *out, reverse=reverse, z0=z0, log_alpha=log_alpha, beta=beta
            )
        return out
